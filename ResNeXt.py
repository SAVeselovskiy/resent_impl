...
with tf.Session() as sess:
    ckpt = tf.train.get_checkpoint_state('../drive/ResNeXt/model')
    if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
        saver.restore(sess, ckpt.model_checkpoint_path)
    else:
        sess.run(tf.global_variables_initializer())

    summary_writer = tf.summary.FileWriter('../drive/ResNeXt/logs', sess.graph)
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        with open('../drive/ResNeXt/logs.txt', 'a') as f:
            f.write(line)

        saver.save(sess=sess, save_path='../drive/ResNeXt/model/ResNeXt.ckpt')
